...
from wordcloud import WordCloud, STOPWORDS  # Import WordCloud dan STOPWORDS
...
def load_data():
    ...
    Sekarang juga memuat kolom 'Text' untuk Word Cloud.
    """
    try:
        # Memuat semua kolom yang relevan, termasuk 'Text'
    ...
    required_columns = ['UserId', 'ProductId', 'Score', 'Text']  # Tambahkan 'Text'
    if not all(col in df.columns for col in required_columns):
        logging.error(f"Error: Missing required columns. Found: {df.columns.tolist()}, Required: {required_columns}")
        return pd.DataFrame()  # Mengembalikan DataFrame kosong jika kolom hilang

    df = df.dropna(subset=required_columns)
    df = df[required_columns]
    df['Score'] = pd.to_numeric(df['Score'], errors='coerce')
    df = df.dropna(subset=['Score'])  # Hapus baris di mana konversi skor gagal
...
def generate_visualizations(df):
    ...
    if df.empty:
        logging.warning("DataFrame kosong, melewati visualisasi.")
    ...
    logging.info("Visualisasi Distribusi Rating berhasil dibuat.")
    ...
    if not df['ProductId'].empty:
        ...
        if not top_products.empty:
            ...
            logging.info("Visualisasi Produk Paling Populer berhasil dibuat.")
        else:
            logging.warning("Tidak ada produk terpopuler untuk divisualisasikan setelah value_counts.")
    else:
        logging.warning("Kolom ProductId kosong, melewati visualisasi Produk Paling Populer.")
    ...
    if not df['UserId'].empty:
        ...
        if not top_reviewers.empty:
            ...
            logging.info("Visualisasi Reviewer Teraktif berhasil dibuat.")
        else:
            logging.warning("Tidak ada reviewer teraktif untuk divisualisasikan setelah value_counts.")
    else:
        logging.warning("Kolom UserId kosong, melewati visualisasi Reviewer Teraktif.")
...
def cluster_users(df, n_clusters=3):
    ...
    if df.empty:
        logging.warning("DataFrame kosong, melewati clustering pengguna.")
    ...
    if user_product_matrix.shape[1] < 2:
        ...
        try:
            ...
            logging.info("KMeans clustering selesai tanpa PCA.")
        except Exception as e:
            logging.error(f"Error selama KMeans clustering tanpa PCA: {e}")
    ...
    try:
        ...
        logging.info("PCA selesai. Direduksi menjadi 2 komponen.")
    except Exception as e:
        logging.error(f"Error selama PCA: {e}. Mungkin tidak cukup titik data atau fitur? {user_product_matrix.shape}")
        return clustered_users_df, cluster_viz
    ...
    try:
        ...
        logging.info("KMeans clustering selesai pada komponen PCA.")
        ...
        plt.xlabel(f'Principal Component 1 ({pca.explained_variance_ratio_[0] * 100:.2f}% Variansi)', fontsize=12)
        plt.ylabel(f'Principal Component 2 ({pca.explained_variance_ratio_[1] * 100:.2f}% Variansi)', fontsize=12)
        ...
        logging.info("Visualisasi Cluster Pengguna menggunakan PCA berhasil dibuat.")

    except Exception as e:
        logging.error(f"Error selama KMeans clustering atau visualisasi: {e}")
...
def generate_recommendations(df):
    ...
    if df.empty:
        logging.warning("DataFrame kosong, melewati pembuatan rekomendasi.")
        return rules
    ...
    try:
        ...
        logging.info(f"DataFrame setelah one-hot encoding shape: {df_encoded.shape}")
    ...
    try:
        ...
        logging.error(f"Error selama fpgrowth: {e}")
        return rules
    ...
    try:
        ...
        logging.error(f"Error selama pembuatan association_rules: {e}")
        return rules
...
def generate_wordcloud(df):
    """
    Menghasilkan word cloud dari kolom teks ulasan.
    """
    wordcloud_image = None
    if df.empty or 'Text' not in df.columns or df['Text'].isnull().all():
        logging.warning("Tidak ada data teks atau kolom 'Text' kosong, melewati pembuatan word cloud.")
        return wordcloud_image

    # Gabungkan semua teks ulasan menjadi satu string
    text = " ".join(review for review in df['Text'].astype(str) if pd.notnull(review))

    if not text.strip():  # Periksa apakah string teks kosong setelah strip
        logging.warning("String teks kosong setelah penggabungan, tidak dapat membuat word cloud.")
        return wordcloud_image

    # Tambahkan stopwords umum dan beberapa kata yang mungkin tidak relevan dari dataset
    stopwords = set(STOPWORDS)
    stopwords.update(
        ["br", "product", "taste", "flavor", "food", "great", "good", "one", "like", "just", "get", "dont", "would",
         "really", "much", "try", "also", "made", "can", "use", "make", "find", "little", "well", "bought", "amazon",
         "amazon.com", "price"])  # Contoh: tambahkan kata-kata umum dari ulasan
    ...
    try:
        ...
        wordcloud = WordCloud(stopwords=stopwords,
                              ...
                              width=1000,
                              height=600,
                              max_words=100,  # Batasi jumlah kata
                              collocations=False  # Nonaktifkan kolokasi untuk fokus pada kata tunggal
                              ).generate(text)

        # Simpan gambar word cloud ke buffer BytesIO
        buffer = BytesIO()
        wordcloud.to_image().save(buffer, format='png')
        plt.close()  # Tutup plot matplotlib
        wordcloud_image = base64.b64encode(buffer.getvalue()).decode('utf-8')
        logging.info("Word cloud berhasil dibuat.")
    except Exception as e:
        logging.error(f"Error saat membuat word cloud: {e}")
...
def index():
    ...
    if not rules.empty:
        ...
        logging.info("Tabel Aturan Asosiasi berhasil dibuat.")
    ...
    if not df.empty:
        ...
        logging.info("Tabel Transaksi untuk Debugging berhasil dibuat.")

    # Buat Word Cloud
    wordcloud_data = generate_wordcloud(df)
    ...
    return render_template('result.html',
                           ...
                           wordcloud_image=wordcloud_data)  # Kirim data word cloud ke template
